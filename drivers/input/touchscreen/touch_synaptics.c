...
static int read_page_description_table(struct i2c_client* client)
{
	...
	unsigned char query[FINGER_QUERY_LEN];
...
int get_ic_info(struct synaptics_ts_data* ts, struct touch_fw_info* fw_info)
{
	...
	if(ts->pdata->panel_type >= 0){
		TOUCH_ERR_MSG("panel_type : (%d)\n",ts->pdata->panel_type);
		strncpy(ts->fw_info.fw_image_product_id, &SynaFirmware[ts->pdata->panel_type-1][16], 10);
		strncpy(ts->fw_info.image_config_id, &SynaFirmware[ts->pdata->panel_type-1][0xb100],4);
		ts->fw_info.fw_start = (unsigned char *)&SynaFirmware[ts->pdata->panel_type-1][0];
		ts->fw_info.fw_size = sizeof(SynaFirmware[ts->pdata->panel_type-1]);
	}else{
		TOUCH_ERR_MSG("get_ic_info : firmware upgrade fail(panel_type : %d)\n",ts->pdata->panel_type);
		return -EIO;
	}
