...
qpnp_chg_usb_chg_gone_irq_handler(int irq, void *_chip)
{
	...
	{	
//		pr_err("chg_gone vbatdet changed to 4.25V\n");
...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	if (chip->usb_present ^ usb_present) {
		...
		chip->is_charger_changed_from_irq = true;
#ifdef CONFIG_LGE_PM_4_25V_CHARGING_START
//			pr_err("qpnp_chg_usb_usbin_valid_irq_handler chip->usb_present=%d,usb_present=%d\n",chip->usb_present,usb_present);
		if(usb_present == 1)
		{
			schedule_delayed_work(&chip->battemp_work, HZ*5);
			/* when the device goes to suspend, the battemp_work isn't work.That's why charging isn't started.
			so we insert pm_stay_awake until starting the battemp work.*/
			pm_stay_awake(chip->dev);
		}
		else schedule_work(&chip->batt_temp_cancel_work);
#else
...
#ifdef CONFIG_LGE_PM_4_25V_CHARGING_START
			chip->from_usbin_valid_irq = true;
			chip->from_temp_monitor_vbat_det_high = false;
...
/*                                        */
    ...
    #ifdef  CONFIG_LGE_PM
    pr_info("[LGE] usb_online: %d\n", ret.intval);

	chip->dc_psy.get_property(&chip->dc_psy,
		      POWER_SUPPLY_PROP_ONLINE, &ret);
	
	pr_info("[LGE] ac_online: %d\n", ret.intval);
...
static void qpnp_monitor_batt_temp(struct work_struct *work)
{
...
#ifdef CONFIG_LGE_PM_4_25V_CHARGING_START
	if((((req.batt_volt/1000) > (chip->max_voltage_mv - chip->resume_delta_mv - PMIC_DEVIATION))
		|| (battery_charging_status != POWER_SUPPLY_STATUS_CHARGING))
		&&(chip->from_usbin_valid_irq == true))
	{
	...
	else if(((req.batt_volt/1000) <= (chip->max_voltage_mv - chip->resume_delta_mv - PMIC_DEVIATION))
		&&(chip->from_usbin_valid_irq == true))
	{
		chip->from_usbin_valid_irq = false;
	}
...
static int qpnp_chg_suspend(struct device *dev)
{
	...
	if(chip->from_temp_monitor_vbat_det_high == true)
	{	
		//pr_err("qpnp_chg_suspend change vbatdet to 4.25V\n");
		qpnp_chg_vbatdet_set(chip, chip->max_voltage_mv - chip->resume_delta_mv);
		chip->from_temp_monitor_vbat_det_high = false;
	}
#endif
