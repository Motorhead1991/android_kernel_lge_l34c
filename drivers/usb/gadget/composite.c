...
#if defined(CONFIG_LGE_PM_VZW_FAST_CHG)
...
static bool usb_connected_flag = false;
static bool usb_configured_flag = false;
static struct delayed_work usb_detect_w;
#define USB_DETECT_DELAY msecs_to_jiffies(30000)
static void usb_detect_work(struct work_struct *w)
{
	if (usb_configured_flag) {
		...
		pr_info("usb_detect_work: USB DRIVER_INSTALLED\n");
	} else {
		set_vzw_usb_charging_state(1 /* IS_USB_DRIVER_UNINSTALLED */);
		pr_info("usb_detect_work: USB DRIVER_UNINSTALLED\n");
	}
}
#endif
...
composite_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
	...
	case USB_REQ_SET_CONFIGURATION:
		...
		spin_unlock(&cdev->lock);
#ifdef CONFIG_LGE_PM_VZW_FAST_CHG
		if (usb_connected_flag && !usb_configured_flag) {
