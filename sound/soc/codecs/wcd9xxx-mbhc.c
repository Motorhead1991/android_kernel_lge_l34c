...
static void wcd9xxx_mbhc_setup(struct wcd9xxx_mbhc *mbhc)
{
	...
	// hj74.kim // partial injection polling noise
	printk("[LGE MBHC] : setup fnt codec_type = %d \n", mbhc->resmgr->codec_type);
	if (mbhc->resmgr->codec_type == WCD9XXX_CDC_TYPE_TAPAN)
...
/*
 * It is possible that this function can be invoked when mbhc_cfg can be NULL
 * Always check if mbhc_cfg is NULL before accessing it to prevent crash
 */
				...
				void *data)
{
	...
	case WCD9XXX_EVENT_POST_BG_MBHC_ON:
		/* Not used for now */
		break;
	case WCD9XXX_EVENT_TX_3_ON:
		/*
		 * if polling is ON, micbias2 not enabled
		 *	switch micbias source to VDDIO
		 */
		set_bit(MBHC_EVENT_TX_3_ON, &mbhc->event_state);
		if (!(snd_soc_read(codec, mbhc->mbhc_bias_regs.ctl_reg)
			  & 0x80) &&
			mbhc->polling_active && !mbhc->mbhc_micbias_switched)
			wcd9xxx_switch_micbias(mbhc, 1);
		break;
	case WCD9XXX_EVENT_TX_3_OFF:
		/*
		 * if polling is ON, micbias2 not enabled
		 *	switch micbias source to VDDIO
		 */
		clear_bit(MBHC_EVENT_TX_3_ON, &mbhc->event_state);
