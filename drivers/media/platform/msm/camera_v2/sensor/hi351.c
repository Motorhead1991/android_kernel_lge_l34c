...
/*                                                                                  */
#if defined(CONFIG_MACH_MSM8X10_W3DS_TIM_BR) || defined(CONFIG_MACH_MSM8X10_W3DS_GLOBAL_COM) || defined(CONFIG_MACH_MSM8X10_W3_VDF_EU) || defined(CONFIG_MACH_MSM8X10_W3_VDF_COM) || defined(CONFIG_MACH_MSM8X10_W3_GLOBAL_COM)
	{
...
static struct msm_camera_i2c_conf_array hi351_prev_conf0[] = {
	{&hi351_prev_settings0[0],
	ARRAY_SIZE(hi351_prev_settings0), 0, MSM_CAMERA_I2C_BYTE_DATA}
};

// conf1 is for after snapshot or restart-previewing.
static struct msm_camera_i2c_conf_array hi351_prev_conf1[] = {
	{&hi351_prev_settings1[0],
	ARRAY_SIZE(hi351_prev_settings1), 0, MSM_CAMERA_I2C_BYTE_DATA}
};
//                                                                                                                             
...
//                                                                                                                             
// conf0 is with recommend setting right after.
static struct msm_camera_i2c_conf_array hi351_auto_fps0_conf[] = {
	{&hi351_auto_fps_settings0[0],
	ARRAY_SIZE(hi351_auto_fps_settings0), 0, MSM_CAMERA_I2C_BYTE_DATA}
};

// conf1 is for after snapshot or restart-previewing.
static struct msm_camera_i2c_conf_array hi351_auto_fps1_conf[] = {
	{&hi351_auto_fps_settings1[0],
	ARRAY_SIZE(hi351_auto_fps_settings1), 0, MSM_CAMERA_I2C_BYTE_DATA}
};
//                                                                                                                             

//                                                                                                                             
// conf0 is with recommend setting right after.
static struct msm_camera_i2c_conf_array hi351_attached_fps0_conf[] = {
	{&hi351_attached_fps_settings0[0],
	ARRAY_SIZE(hi351_attached_fps_settings0), 0, MSM_CAMERA_I2C_BYTE_DATA}
};

// conf1 is for after snapshot or restart-previewing.
static struct msm_camera_i2c_conf_array hi351_attached_fps1_conf[] = {
	{&hi351_attached_fps_settings1[0],
	ARRAY_SIZE(hi351_attached_fps_settings1), 0, MSM_CAMERA_I2C_BYTE_DATA}
};
//                                                                                                                             

//                                                                                                                             
// conf0 is with recommend setting right after.
static struct msm_camera_i2c_conf_array hi351_fixed_fps0_conf[] = {
	{&hi351_fixed_fps_settings0[0],
	ARRAY_SIZE(hi351_fixed_fps_settings0), 0, MSM_CAMERA_I2C_BYTE_DATA}
};

// conf1 is for after snapshot or restart-previewing.
static struct msm_camera_i2c_conf_array hi351_fixed_fps1_conf[] = {
	{&hi351_fixed_fps_settings1[0],
	ARRAY_SIZE(hi351_fixed_fps_settings1), 0, MSM_CAMERA_I2C_BYTE_DATA}
};
//                                                                                                                             
...
static void hi351_set_scene_mode(struct msm_sensor_ctrl_t *s_ctrl, int value)
{
	...
	case MSM_CAMERA_SCENE_MODE_SPORTS: {
		hi351_i2c_write_table(s_ctrl, &hi351_reg_scene_sport[0],
			ARRAY_SIZE(hi351_reg_scene_sport));
		hi351_i2c_write_table(s_ctrl, &hi351_scene_active_settings[0],
			ARRAY_SIZE(hi351_scene_active_settings));
		break;
	}
	case MSM_CAMERA_SCENE_MODE_SUNSET: {
		hi351_i2c_write_table(s_ctrl, &hi351_reg_scene_sunset[0],
			ARRAY_SIZE(hi351_reg_scene_sunset));
		hi351_i2c_write_table(s_ctrl, &hi351_scene_active_settings[0],
			ARRAY_SIZE(hi351_scene_active_settings));
		break;
	}
	...
	switch (value) {
		case 0: { //attached MMS VIDEO 177x144, 384x288
			...
			rc = hi351_sensor_write_init_settings(s_ctrl->sensor_i2c_client,
					(struct msm_camera_i2c_reg_conf *) hi351_attached_fps0_conf[0].conf, hi351_attached_fps0_conf[0].size);
				}
			// conf1 is normal situation.
			else if(INIT_DONE == 0){
			rc = hi351_sensor_write_init_settings(s_ctrl->sensor_i2c_client,
					(struct msm_camera_i2c_reg_conf *) hi351_attached_fps1_conf[0].conf, hi351_attached_fps1_conf[0].size);
				}
			//                                                                                                                             
			}
			break;
		...
		case 1: { //fixed for VIDEO 640x480 and more over.
			...
			if(INIT_DONE == 1){
			rc = hi351_sensor_write_init_settings(s_ctrl->sensor_i2c_client,
					(struct msm_camera_i2c_reg_conf *) hi351_fixed_fps0_conf[0].conf, hi351_fixed_fps0_conf[0].size);
				}
			// conf1 is normal situation.
			else if(INIT_DONE == 0){
			rc = hi351_sensor_write_init_settings(s_ctrl->sensor_i2c_client,
					(struct msm_camera_i2c_reg_conf *) hi351_fixed_fps1_conf[0].conf, hi351_fixed_fps1_conf[0].size);
					}
			//                                                                                                                             
			}
			break;
		...
		default:{  //default
			...
			if(INIT_DONE == 1){
			rc = hi351_sensor_write_init_settings(s_ctrl->sensor_i2c_client,
					(struct msm_camera_i2c_reg_conf *) hi351_auto_fps0_conf[0].conf, hi351_auto_fps0_conf[0].size);
				}
			// conf1 is normal situation.
			else if(INIT_DONE == 0){
			rc = hi351_sensor_write_init_settings(s_ctrl->sensor_i2c_client,
					(struct msm_camera_i2c_reg_conf *) hi351_auto_fps1_conf[0].conf, hi351_auto_fps1_conf[0].size);
					}
			//                                                                                                                             
			}
			break;
	...
	void __user *argp)
{
	...
	case CFG_SET_INIT_SETTING: {
		...
		int32_t retry;
		int32_t i;
		pr_err("%s, CFG_SET_INIT_SETTING!! start\n", __func__);
	...
	case CFG_SET_RESOLUTION: {
		...
		if (val == 0){
			int32_t rc = 0;
			rc = hi351_sensor_write_init_settings(s_ctrl->sensor_i2c_client,
							(struct msm_camera_i2c_reg_conf *) hi351_snap_conf[0].conf, hi351_snap_conf[0].size);
			...
			if(INIT_DONE == 1){
				rc = hi351_sensor_write_init_settings(s_ctrl->sensor_i2c_client,
					(struct msm_camera_i2c_reg_conf *) hi351_prev_conf0[0].conf, hi351_prev_conf0[0].size);
					pr_err("%s, prevsettings!! Changed INIT_DONE : %d\n", __func__, INIT_DONE);
					break;
			}
			// conf1 is normal situation.
			if(INIT_DONE == 0){
				rc = hi351_sensor_write_init_settings(s_ctrl->sensor_i2c_client,
					(struct msm_camera_i2c_reg_conf *) hi351_prev_conf1[0].conf, hi351_prev_conf1[0].size);
					pr_err("%s, prevsettings!! INIT_DONE hi351_prev_conf1 original : %d\n", __func__, INIT_DONE);
					break;
			}
			//                                                                                                                             
		}
	...
	case CFG_SET_START_STREAM:
		pr_debug("%s, CFG_SET_START_STREAM!!\n", __func__);
		//                                                                                                                             
		...
		if(INIT_DONE == 1){
		hi351_i2c_write_table(s_ctrl,
			&hi351_start_settings0[0],
			ARRAY_SIZE(hi351_start_settings0));
		...
		hi351_i2c_write_table(s_ctrl,
			&hi351_start_settings1[0],
			ARRAY_SIZE(hi351_start_settings1));
			}
		//                                                                                                                             
		break;
