...
{
#ifdef CONFIG_LGE_OTG_CABLE_ADC_DETECTING
	...
	case USB_OTG:
		if (pdata->otg_control == OTG_USER_CONTROL) {
			...
			if(!test_bit(ID, &motg->inputs)) {
				if ( !motg->vadc_dev )
					pr_debug("msm_otg_init_sm : vadc_dev is not ready\n");
				else {
					usb_adc_val = lge_pm_get_cable_usb_id_adc(motg);
					lge_pm_read_cable_info(usb_adc_val);
				}

				if(lge_pm_get_cable_type() != CABLE_OTG) {
					pr_debug("msm_otg_init_sm : OTG_Cable is not connected\n");
					set_bit(ID, &motg->inputs);
				}
				else {
					pr_debug("msm_otg_init_sm : OTG_Cable is connected\n");
				}
			}
#endif //jaegeun.jung
...
#ifdef CONFIG_MACH_MSM8x10_W3C_VZW
					msm_otg_notify_charger(motg,
							IDEV_CHG_CDP);
#else
					msm_otg_notify_charger(motg,
...
static void msm_pmic_id_status_w(struct work_struct *w)
{
	...
	if(!test_bit(ID, &motg->inputs)) {
		if ( !motg->vadc_dev )
			pr_debug("msm_pmic_id_status_w : vadc_dev is not ready\n");
		else {
			usb_adc_val = lge_pm_get_cable_usb_id_adc(motg);
			lge_pm_read_cable_info(usb_adc_val);
		}
	    ...
	    if(lge_pm_get_cable_type() != CABLE_OTG) {
			set_bit(ID, &motg->inputs);
			if(!test_bit(B_SESS_VLD, &motg->inputs)) {
				pr_debug("msm_pmic_id_status_w : !OTG_Cable ,!B_SESS_VLD Ignore queue sm_work\n");
				work = 0;
			}
			else {
				pr_debug("msm_pmic_id_status_w : !OTG_Cable ,B_SESS_VLD queue sm_work\n");
			}
		}
    }
#endif //jaegeun.jung
